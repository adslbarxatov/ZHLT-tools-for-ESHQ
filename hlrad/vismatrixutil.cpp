...
static transfer_index_t *CompressTransferIndicies (transfer_raw_index_t *tRaw, const unsigned rawSize, unsigned *iSize)
	...
	transfer_raw_index_t *end = tRaw + rawSize - 1;
	// -1 since we are comparing current with next and get errors when bumping into the 'end'

#ifndef HLRAD_HULLU_COMPRESSEDARRAY_TWEAK
	transfer_index_t CompressedArray[MAX_PATCHES];
	// somewhat big stack object (1 Mb with 256k patches)
#endif
