...
bool		g_chart_rad = DEFAULT_CHART;		// ESHQ: разделение имЄн
bool		g_estimate_rad = DEFAULT_ESTIMATE;	// ESHQ: разделение имЄн
bool		g_info_rad = DEFAULT_INFO;			// ESHQ: разделение имЄн
...
// =====================================================================================
void GetParamsFromEnt_RAD (entity_t *mapent)
	{
	int			iTmp;
	float		flTmp;
	char		szTmp[256];
	...
	if (IntForKey (mapent, "estimate"))
		...
		{
		g_estimate_rad = false;
		}
	Log ("%30s [ %-9s ]\n", "Estimate Compile Times", g_estimate_rad ? "on" : "off");
	...
	if (pszTmp)
		...
		if (sscanf_s (pszTmp, "%f %f %f", &red, &green, &blue))
			...
			if (red < 0 || red > 1 || green < 0 || green > 1 || blue < 0 || blue > 1)
				{
				Error ("info_compile_parameters: Ambient World Light (ambient) all 3 values "
...
// ESHQ: участок отключЄн, т.к. дублирует уже существующий в cmdlib
#if 0
// =====================================================================================
...
static void SubdividePatch (patch_t *patch)
	...
	// ATTENTION: We let SortPatches relink all the ->next correctly! instead of doing
	// it here too which is somewhat complicated
	}
	...
	for (j = 0, patch = g_patches; j < g_num_patches; j++, patch++)
		...
		for (i = 0; i < w->m_NumPoints; i++)
			...
			Log ("%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n",
				w->m_Points[i][0],
				w->m_Points[i][1],
				w->m_Points[i][2], patch->totallight[0][0] / 256, patch->totallight[0][1] / 256,
				patch->totallight[0][2] / 256); //LRC
#else
			Log ("%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n",
				w->m_Points[i][0],
				w->m_Points[i][1],
				w->m_Points[i][2], patch->totallight[0] / 256, patch->totallight[1] / 256,
...
// ESHQ: отключЄн вывод в консоль
		...
		if (g_rgb_transfers)
			{
			NamedRunThreadsOn (g_num_patches, g_estimate_rad, GatherRGBLight);
			}
		else
			{
			NamedRunThreadsOn (g_num_patches, g_estimate_rad, GatherLight);
			}
#else
		NamedRunThreadsOn (g_num_patches, g_estimate_rad, GatherLight);
#endif
...
static void RadWorld ()
	...
	NamedRunThreadsOnIndividual (g_numfaces, g_estimate_rad, BuildFacelights);
	...
	NamedRunThreadsOnIndividual (g_numfaces, g_estimate_rad, FinalLightFace);
	}

// ESHQ: не требуетс€ в подчинЄнном режиме
#if CON_UI
// =====================================================================================
// Usage
// =====================================================================================
...
static void Settings ()
	...
	Log ("chart                [ %17s ] [ %17s ]\n", g_chart_rad ? "on" : "off", DEFAULT_CHART ? "on" : "off");
	Log ("estimate             [ %17s ] [ %17s ]\n", g_estimate_rad ? "on" : "off", DEFAULT_ESTIMATE ? "on" : "off");
	...
	safe_snprintf (buf2, sizeof (buf2), "%1.3f %1.3f %1.3f", DEFAULT_AMBIENT_RED, DEFAULT_AMBIENT_GREEN,
...
#ifdef SYSTEM_WIN32
	GetModuleFileNameA (NULL, tmp, _MAX_PATH);	// ESHQ: исправление кодировки
#else
...
// ESHQ: отключЄн штатный интерфейс командной строки
#if CON_UI
// =====================================================================================
// main
// =====================================================================================
...
//
// ESHQ: сводный упрощЄнный интерфейс дл€ модул€ RAD
// 
// Mode 0: финальна€ компил€ци€ дл€ ESHQ
// Mode 1: быстра€ компил€ци€ дл€ ES:RM
//
int MakeRAD (const char *Map, unsigned __int8 Mode)
	{
	int			i;
	double		start, end;
	const char	*mapname_from_arg = NULL;
	...
	switch (Mode)
		...
		default:
			g_extra = true;
			g_colour_qgamma[0] = 0.45f;
			g_colour_qgamma[1] = 0.45f;
			g_colour_qgamma[2] = 0.45f;
			break;
	...
	char mapName[1][MAX_PATH];
	mapName[0][0] = '\0';
	sprintf_s (mapName[0], "%s", Map);

	LogStart (1, (char **)mapName);
	...
	ParseEntities ();
	Settings ();
	LoadRadFiles (g_Mapname, user_lights, NULL);	// ESHQ: последний параметр не используетс€
