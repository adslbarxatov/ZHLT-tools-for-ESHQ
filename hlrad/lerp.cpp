...
// =====================================================================================
static bool TestWallIntersectTri (const lerpTriangulation_t *const trian, const vec3_t p1, const vec3_t p2,
	const vec3_t p3)
	{
	int		x;
...
// =====================================================================================
#ifdef ZHLT_TEXLIGHT
static void LerpTriangle (const lerpTriangulation_t *const trian, const vec3_t point, vec3_t result,
	const unsigned pt1, const unsigned pt2, const unsigned pt3, int style) //LRC
#else
static void LerpTriangle (const lerpTriangulation_t *const trian, const vec3_t point, vec3_t result,
	const unsigned pt1, const unsigned pt2, const unsigned pt3)
#endif
	{
	patch_t	*p1;
	patch_t	*p2;
	...
	if (!TestLineSegmentIntersectWall (trian, p1->origin, p2->origin))
		...
		if (d >= ON_EPSILON)
			...
			for (i = 0; i < 3; i++)
				{
#ifdef ZHLT_TEXLIGHT
				result[i] = (((*GetTotalLight (p1, style))[i] * length2) + ((*GetTotalLight (p1, style))[i] * length1)) /
					total_length; //LRC
#else
	...
	if (!TestLineSegmentIntersectWall (trian, p1->origin, p3->origin))
		...
		if (d >= ON_EPSILON)
			...
			for (i = 0; i < 3; i++)
				{
#ifdef ZHLT_TEXLIGHT
				result[i] = (((*GetTotalLight (p1, style))[i] * length2) + ((*GetTotalLight (p3, style))[i] * length1)) /
					total_length; //LRC
...
static void AddPatchToTriangulation (lerpTriangulation_t * trian, patch_t * patch)
	...
	if (!(patch->flags & ePatchFlagOutside))
		...
		if (pnum >= trian->maxpoints)
			...
			memset (trian->points + trian->maxpoints, 0, sizeof (patch_t *) * DEFAULT_MAX_LERP_POINTS);
			// clear the new block
	...
	for (x = 0; x < face->numedges; x++)
		...
		if (f2 && !es->coplanar && VectorCompare (vec3_origin, es->interface_normal))
			...
			if (trian->numwalls >= trian->maxwalls)
				...
				memset (trian->walls + trian->maxwalls, 0, sizeof (lerpWall_t) * DEFAULT_MAX_LERP_WALLS);
				// clear the new block
