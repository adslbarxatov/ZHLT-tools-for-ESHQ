...
bool			g_estimate_vis = DEFAULT_ESTIMATE;	// ESHQ: разделение имён
bool			g_chart_vis = DEFAULT_CHART;		// ESHQ: разделение имён
bool			g_info_vis = DEFAULT_INFO;			// ESHQ: разделение имён
...
volatile int	g_visportalindex = UNINITIALIZED_PORTAL_INDEX;
// a client's portal index : current portalindex being worked on
...
volatile unsigned long	g_idletime = 0;
// Accumulated idle time in milliseconds (rolls over after 46.7 days, hopefully a vis client wont run that long)
volatile unsigned long	g_serverindex = 0;
// client only variable, server index for calculating percentage indicators on the client
...
char	*g_bsp_image = NULL;
// Client/Server variable : Server uses it for cache for connecting clients, clients download
// it to memory to not require filesystem usage 
char	*g_prt_image = NULL;
// Client/Server variable : Server uses it for cache for connecting clients, clients download
// it to memory to not require filesystem usage 
...
// =====================================================================================
void GetParamsFromEnt_VIS (entity_t *mapent)
	{
	int		iTmp;
	...
	if (IntForKey (mapent, "estimate"))
		...
		{
		g_estimate_vis = false;
		}
	Log ("%30s [ %-9s ]\n", "Estimate Compile Times", g_estimate_vis ? "on" : "off");
...
static portal_t *GetNextPortal ()
	...
	while (getWorkFromClientQueue () == WAITING_FOR_PORTAL_INDEX)
		...
		g_idletime += delay;
		// This is the only point where the portal work goes idle,
		// so its easy to add up just how idle it is
	...
	for (i = 0; i < leaf->numportals; i++)
		...
		if ((tmp == 0) && (outbuffer[offset] & bit))
			...
			for (k = 0; k < p->winding->numpoints; k++)
				{
				Log ("    (%4.3f %4.3f %4.3f)\n", p->winding->points[k][0], p->winding->points[k][1],
...
static void CalcPortalVis ()
	...
	LeafThread (0);
#else
	NamedRunThreadsOn (g_numportals * 2, g_estimate_vis, LeafThread);
#endif
	}
...
static void CalcVis ()
	...
	NamedRunThreadsOn (g_numportals * 2, g_estimate_vis, BasePortalVis);
	...
	if (g_maxdistance)
		...
		NamedRunThreadsOn (g_portalleafs, g_estimate_vis, MaxDistVis);
...
static void Settings ()
	...
	Log ("chart               [ %7s ] [ %7s ]\n", g_chart_vis ? "on" : "off", DEFAULT_CHART ? "on" : "off");
	Log ("estimate            [ %7s ] [ %7s ]\n", g_estimate_vis ? "on" : "off", DEFAULT_ESTIMATE ? "on" : "off");
...
// ESHQ: сводный упрощённый интерфейс для модуля VIS
int MakeVIS (const char *Map, unsigned __int8 Mode)
	{
	char		portalfile[_MAX_PATH];
	char		source[_MAX_PATH];
	int			i;
