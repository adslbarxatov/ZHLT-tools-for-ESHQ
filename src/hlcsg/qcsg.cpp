...
bool		g_noclip_csg = DEFAULT_NOCLIP;			// no clipping hull "-noclip"; ESHQ: разделение имён
bool		g_onlyents = DEFAULT_ONLYENTS;			// onlyents mode "-onlyents"
bool		g_wadtextures = DEFAULT_WADTEXTURES;	// "-nowadtextures"
bool		g_chart_csg = DEFAULT_CHART;			// show chart "-chart"; ESHQ: разделение имён
bool		g_skyclip = DEFAULT_SKYCLIP;			// no sky clipping "-noskyclip"
bool		g_estimate_csg = DEFAULT_ESTIMATE;		// progress estimates "-estimate"; ESHQ: разделение имён
bool		g_info_csg = DEFAULT_INFO;				// -info; ESHQ: разделение имён
const char	*g_hullfile = NULL;						// external hullfile "-hullfie sdfsd"

#ifdef ZHLT_NULLTEX // AJM
bool		g_bUseNullTex_csg = DEFAULT_NULLTEX;	// "-nonulltex"; ESHQ: разделение имён
#endif
...
// =====================================================================================
void GetParamsFromEnt_CSG (entity_t *mapent)
	{
	int		iTmp;
	char	szTmp[256];
	...
	if (IntForKey (mapent, "estimate"))
		...
		{
		g_estimate_csg = false;
		}
	Log ("%30s [ %-9s ]\n", "Estimate Compile Times", g_estimate_csg ? "on" : "off");
	...
	Log ("%30s [ %-9s ]\n", "Clipping Hull Generation", g_noclip_csg ? "off" : "on");
...
static void ProcessModels ()
	...
	for (i = 0; i < g_numentities; i++)
		...
		if (i == 0) // if its worldspawn....
			{
			NamedRunThreadsOnIndividual (g_entities[i].numbrushes, g_estimate_csg, CSGBrush);
			CheckFatal ();
			}
		else
			{
...
static void Settings ()
	...
	Log ("chart                 [ %7s ] [ %7s ]\n", g_chart_csg ? "on" : "off", DEFAULT_CHART ? "on" : "off");
	Log ("estimate              [ %7s ] [ %7s ]\n", g_estimate_csg ? "on" : "off", DEFAULT_ESTIMATE ? "on" : "off");
	...
	Log ("noclip                [ %7s ] [ %7s ]\n", g_noclip_csg ? "on" : "off", DEFAULT_NOCLIP ? "on" : "off");

#ifdef ZHLT_NULLTEX // AJM:
	Log ("null texture stripping[ %7s ] [ %7s ]\n", g_bUseNullTex_csg ? "on" : "off", DEFAULT_NULLTEX ? "on" : "off");
#endif
...
int main (const int argc, char **argv)
	...
	// createbrush
	NamedRunThreadsOnIndividual (g_nummapbrushes, g_estimate_csg, CreateBrush);
	CheckFatal ();
	...
	// Set model centers
	NamedRunThreadsOnIndividual (g_numentities, g_estimate_csg, SetModelCenters);

	// Calc brush unions
	if ((g_BrushUnionThreshold > 0.0) && (g_BrushUnionThreshold <= 100.0))
		{
		NamedRunThreadsOnIndividual (g_nummapbrushes, g_estimate_csg, CalculateBrushUnions);
		}
...
// ESHQ: сводный упрощённый интерфейс для модуля CSG
int MakeCSG (const char *Map, unsigned __int8 Mode)
	{
	int			i;
	char		name[_MAX_PATH];			// mapanme 
	double		start, end;					// start/end time log
	...
	switch (Mode)
		...
		default:
			g_cliptype = clip_precise;
			g_WadInclude.push_back ("eshq68.wad");
			break;
	...
	if (g_onlyents)
		{
		safe_snprintf (out2, _MAX_PATH, "%s.bsp", g_Mapname);
		LoadBSPFile (out2);
		LoadWadincludeFile (g_Mapname);
	...
	for (i = 0; i < NUM_HULLS; i++)
		{
		safe_snprintf (name2, _MAX_PATH, "%s.p%i", g_Mapname, i);

		out[i] = fopen (name2, "w");
		if (!out[i])
			Error ("Couldn't open %s", name2);
		}
