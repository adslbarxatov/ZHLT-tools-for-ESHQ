...
bool			g_noclip_bsp = DEFAULT_NOCLIP;	// no clipping hull "-noclip"; ESHQ: разделение имён
bool			g_chart_bsp = DEFAULT_CHART;	// print out chart? "-chart"; ESHQ: разделение имён
bool			g_estimate_bsp = DEFAULT_ESTIMATE;	// estimate mode "-estimate"; ESHQ: разделение имён
bool			g_info_bsp = DEFAULT_INFO;		// ESHQ: разделение имён
...
bool			g_bUseNullTex_bsp = DEFAULT_NULLTEX;	// "-nonulltex"; ESHQ: разделение имён
#endif
...
// =====================================================================================
void GetParamsFromEnt_BSP (entity_t *mapent)
	{
	int iTmp;
	...
	if (IntForKey (mapent, "estimate"))
		...
		{
		g_estimate_bsp = false;
		}
	Log ("%30s [ %-9s ]\n", "Estimate Compile Times", g_estimate_bsp ? "on" : "off");
	...
	Log ("%30s [ %-9s ]\n", "Clipping Hull Generation", g_noclip_bsp ? "off" : "on");
...
static void Settings ()
	...
	Log ("chart               [ %7s ] [ %7s ]\n", g_chart_bsp ? "on" : "off", DEFAULT_CHART ? "on" : "off");
	Log ("estimate            [ %7s ] [ %7s ]\n", g_estimate_bsp ? "on" : "off", DEFAULT_ESTIMATE ? "on" : "off");
...
#ifdef HLBSP_THREADS // AJM
	NamedRunThreadsOnIndividual (nummodels, g_estimate_bsp, ProcessModel);
#else
...
// ESHQ: сводный упрощённый интерфейс для модуля BSP
int MakeBSP (const char *Map, unsigned __int8 Mode)
	{
	int			i;
	double		start, end;
	const char	*mapname_from_arg = NULL;
