...
void			BuildFacelights (const int facenum)
	...
	for (i = 0; i < l.numsurfpt; i++, spot += 3)
		...
		for (j = 0; j < MAXLIGHTMAPS && (f->styles[j] != 255); j++)
			...
			if (b_transparency_loss)
				{
				VectorScale (facelight[facenum].samples[j][i].light, light_left_for_facelight,
	...
	if (g_face_patches[facenum])
		...
		if (j == MAXLIGHTMAPS)
			...
			if (!g_warned_direct || g_verbose)
				{
				Warning ("Too many light styles on a face(%f,%f,%f)", g_face_patches[facenum]->origin[0],
					g_face_patches[facenum]->origin[1], g_face_patches[facenum]->origin[2]);
				g_warned_direct = true;
				}
			}
		else
		...
		if (patch)
			...
			for (i = 0; i < MAXLIGHTMAPS && patch->totalstyle[i] != 255; i++)
				...
				if (lightstyles == MAXLIGHTMAPS)
					...
					if (!g_warned_direct || g_verbose)
						{
						Warning ("Too many direct light styles on a face(%f,%f,%f)", patch->origin[0],
