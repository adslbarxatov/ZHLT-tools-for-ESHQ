...
static void BuildVisMatrix ()
	...
	NamedRunThreadsOn (g_numleafs - 1, g_estimate_rad, BuildVisLeafs);
	}
...
#ifndef HLRAD_HULLU
		NamedRunThreadsOn (g_num_patches, g_estimate_rad, MakeScales);
#else
		if (g_rgb_transfers)
			{
			NamedRunThreadsOn (g_num_patches, g_estimate_rad, MakeRGBScales);
			}
		else
			{
			NamedRunThreadsOn (g_num_patches, g_estimate_rad, MakeScales);
...
#ifndef HLRAD_HULLU
		NamedRunThreadsOnIndividual (g_num_patches, g_estimate_rad, SwapTransfers);
#else
		if (g_rgb_transfers)
			{
			NamedRunThreadsOnIndividual (g_num_patches, g_estimate_rad, SwapRGBTransfers);
			}
		else
			{
			NamedRunThreadsOnIndividual (g_num_patches, g_estimate_rad, SwapTransfers);
