...
bool		g_chart_rad = DEFAULT_CHART;		// ESHQ: разделение имён
bool		g_estimate_rad = DEFAULT_ESTIMATE;	// ESHQ: разделение имён
bool		g_info_rad = DEFAULT_INFO;			// ESHQ: разделение имён
...
// ESHQ: участок отключён, т.к. дублирует уже существующий в cmdlib
#if 0
// =====================================================================================
...
// ESHQ: не требуется в подчинённом режиме
#if CON_UI
...
// ESHQ: отключён штатный интерфейс командной строки
#if CON_MAIN
...
// ESHQ: сводный упрощённый интерфейс для модуля RAD
int MakeRAD (const char *Map, unsigned __int8 Mode)
	...
	LoadBSPFile (g_source);
	ParseEntities ();
	LoadRadFiles (g_Mapname, user_lights, NULL);	// ESHQ: последний параметр не используется
