...
void Winding::RemoveColinearPoints ()
	...
	// TODO: OPTIMIZE: this could be 1/2 the number of vectornormalize calls by caching 
	// one of the previous values through the loop
...
void Winding::Clip (const vec3_t normal, const vec_t dist, Winding **front, Winding **back)
	...
	maxpts = m_NumPoints + 4;
	// can't use counts[0]+2 because
	...
	*back = b;
	...
	// | instead of || for branch optimization
	if ((f->m_NumPoints > MAX_POINTS_ON_WINDING) | (b->m_NumPoints > MAX_POINTS_ON_WINDING))
		{
